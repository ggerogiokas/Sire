...
class SubSample(object):
    ...
    def subsample_gradients(self):
        if not self.subsample:
            print(
                "# Eliminating samples from the beginning of the gradient data the data will still be highly correlated!")
            ...
            if N_max < 100:
                warnings.warn(
                    "You have reduced your data to less than 100 samples (%d), the results from these might not "
                    "be trustworthy. " % N_max)
        else:
            print(
                "# We are doing a timeseries analysis using the timeseries analysis module in pymbar and will subsample"
            ...
            if N_max < 100:
                warnings.warn(
                    "You have reduced your gradient data to less than 100 samples (%d), the results from these might not "
                    "be trustworthy. " % N_max)
    ...
    def subsample_energies(self):
        if not self.subsample:
            print(
                "# Eliminating samples from the beginning of the energy data the data will still be highly correlated!")
            ...
            N_max = np.max(self._subsampled_N_k_energies)
            if N_max < 100:
                warnings.warn("You have reduced your energy data to less than 100 samples, the results from these might not "
                              "be trustworthy. ")
            self._subsampled_u_kln = np.zeros([self._gradients_kn.shape[0], self._gradients_kn.shape[0], N_max],
                                              np.float64)
            for k in range(self._gradients_kn.shape[0]):
    ...
    def load_data(self):
        ...
        if self.lam is not None:
            if num_inputfiles != lam.shape[0]:
                raise Exception(
                    "The lambda array you supplied does not have the same length as the number of input files")
                sys.exit(-1)
            ...
            if i == 0:
                ...
                if self.lam is not None:
                    if not np.array_equal(lam_array, self.lam):
                        raise Exception(
                            "Alchemical array provided via the command line does not match the array found in %s" %
                            self.sim_files[0])
                ...
                if not np.array_equal(la, lam_array):
                    raise Exception("Alchemical lambda array provided in %s does not match the array in %s" % (
                        self.sim_files[i], self.sim_files[0]))
                    sys.exit(-1)
